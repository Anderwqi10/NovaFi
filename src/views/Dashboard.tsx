...
const Dashboard: React.FC<{}> = () => {
  ...
  useEffect(() => {
    ...
    let isOverCardsContainer = false;
    
    // Track when mouse is over the cards container
    const handleMouseEnter = () => {
      isOverCardsContainer = true;
    };
    
    const handleMouseLeave = () => {
      isOverCardsContainer = false;
    };
    
    const wheelHandler = (e: WheelEvent) => {
      // Only handle wheel events when mouse is over the cards container
      if (!element || !isOverCardsContainer) {
        return; // Let the event propagate normally for vertical scrolling
      }
      
      // Check if the element can scroll horizontally
      const canScrollHorizontally = element.scrollWidth > element.clientWidth;
      
      // Only prevent default if we're actually scrolling the cards container horizontally
      ...
      // Otherwise, let the scroll event propagate for normal vertical scrolling
    };
  ...
  return (
    <div className="w-full">
      <Tabs />
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 sm:px-8 md:px-12 lg:px-20 mt-4 sm:mt-0">
        <div className="justify-center w-full sm:w-3/5 text-center">
          {loading ? (
            <div className="flex items-center justify-center w-full sm:w-48 text-white bg-[#259da822] rounded p-2 mx-auto">
              Loading...
            </div>
          ) : (
            <div className="flex items-center justify-center w-full sm:w-48 text-white bg-[#259da822] rounded p-2 mx-auto">
      ...
      <div
        className="grid grid-flow-col auto-cols-[100%] grid-rows-none gap-4 mt-10 card-data sm:auto-cols-[35%] md:auto-cols-[20%] lg:auto-cols-[20%] xl:auto-cols-[20%] 2xl:auto-cols-[20%] !overflow-x-auto w-full px-4 sm:px-8"
        id="cards-data"
        style={{
          height: "450px",
          overflowY: "visible",
      ...
      <div className="flex items-center px-4 sm:px-8 md:px-12 lg:px-20 my-8 sm:my-16">
        <div className="flex justify-center w-full gap-4 sm:gap-8">
          <Back
            className="rotate-180 cursor-pointer stroke-white fill-white w-6 h-6 sm:w-8 sm:h-8"
            onClick={() => scrollCards("left")}
          />
          <Back
            className="cursor-pointer stroke-white fill-white w-6 h-6 sm:w-8 sm:h-8"
            onClick={() => scrollCards("right")}
          />
        </div>
      </div>
      <div className="flex flex-col items-center w-full gap-6 sm:gap-10 px-4 sm:px-8 md:px-12 lg:px-20 my-8 sm:my-10">
        <div className="flex items-center justify-center w-full text-2xl sm:text-3xl md:text-4xl text-white">
          LEADERBOARD
        </div>
        <div className="flex flex-col sm:flex-row items-center justify-center w-full text-white gap-4">
          <div className="flex flex-col sm:flex-row items-center w-full sm:w-1/2 gap-3">
            <select
              value={age}
              onChange={handleChange}
              className="w-full sm:w-auto rounded border border-white/20 bg-transparent px-3 py-2 text-white outline-none focus:ring-2 focus:ring-white/30"
            >
            ...
            <div className="relative w-full sm:w-auto sm:flex-1">
              ...
              <button
                type="button"
                className="absolute inset-y-0 right-1 my-1 inline-flex items-center rounded bg-white px-3 text-black hover:bg-gray-200 text-sm"
              >
                Abc
              </button>
            </div>
          </div>
          <div className="flex items-center justify-center w-full sm:w-1/2"></div>
        </div>
      </div>
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-4 md:gap-6 lg:gap-8 px-4 sm:px-8 md:px-12 lg:px-20 my-8 sm:my-10">
        <WinnerCardComponent
          username="abc"
          imageUrl="abc"
        />
      ...
      <div className="flex flex-col items-center px-4 sm:px-8 md:px-12 lg:px-20 overflow-x-auto w-full">
        <WinnerTable />
      </div>
    </div>
  );
