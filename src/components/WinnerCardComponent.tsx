...
const WinnerCardComponent = ({
  ...
  return (
    <div
      className={`w-full sm:w-[28rem] md:w-[32rem] min-h-[18rem] sm:h-[20rem] md:h-[22rem] p-4 sm:p-6 flex flex-col justify-between bg-gradient-to-br rounded-xl relative ${additionalStyles}`}
      style={{
        background:
      ...
      <div className="flex gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-1 min-w-0">
          <img
            ...
            alt="Profile"
            className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 border-2 sm:border-4 border-white rounded-full flex-shrink-0"
          />
          <div className="flex items-center text-xl sm:text-2xl md:text-3xl lg:text-4xl text-white font-poppins truncate">
            {username}
          </div>
        </div>
      </div>

      <div className="flex flex-col gap-2 sm:gap-3 mt-4 sm:mt-0">
        <div className="flex items-center justify-between gap-2">
          <div className="text-sm sm:text-base md:text-lg text-white font-poppins">Win Rate</div>
          <div className="text-base sm:text-lg md:text-xl text-white whitespace-nowrap">{winRate}</div>
        </div>
        <div className="flex items-center justify-between gap-2">
          <div className="text-sm sm:text-base md:text-lg text-white font-poppins">
            Net Winnings (LMNG)
          </div>
          <div className="text-right">
            <div className="text-base sm:text-lg md:text-xl text-green-500 whitespace-nowrap">{netWinnings}</div>
            <div className="flex items-center justify-end text-xs sm:text-sm md:text-base text-white">
              +235
            </div>
          </div>
        </div>
        <div className="flex items-center justify-between gap-2">
          <div className="text-sm sm:text-base md:text-lg text-white font-poppins">Rounds Won</div>
          <div className="text-base sm:text-lg md:text-xl text-white whitespace-nowrap">{roundsWon}</div>
        </div>
      </div>
    </div>
  );
